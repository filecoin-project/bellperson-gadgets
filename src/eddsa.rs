...
use crate::jubjub::{
    edwards::Point, FixedGenerators, JubjubEngine, JubjubParams, ToUniform, Unknown,
};
...
impl<E: JubjubEngine> PrivateKey<E> {
    ...
    pub fn sign_schnorr_blake2s<R: Rng>(
        ...
        r_g_x
            .into_repr()
            .write_le(&mut r_g_x_bytes[..])
            .expect("has serialized r_g_x");
    ...
    pub fn sign<R: Rng>(
        ...
        r_g_y
            .into_repr()
            .write_le(&mut r_g_y_bytes[..])
            .expect("has serialized r_g_y");
        ...
        pk_y.into_repr()
            .write_le(&mut pk_y_bytes[..])
            .expect("has serialized pk_y");

        let concatenated: Vec<u8> = r_g_x_bytes
            .iter()
            .chain(r_g_y_bytes.iter())
...
#[cfg(test)]
mod baby_tests {
    use crate::alt_babyjubjub::{edwards, fs::Fs, AltJubjubBn256, FixedGenerators};
