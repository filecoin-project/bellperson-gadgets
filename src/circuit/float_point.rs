...
pub fn parse_with_exponent_le<E: Engine, CS: ConstraintSystem<E>>(
    ...
    let one_allocated = AllocatedNum::alloc(cs.namespace(|| "allocate one"), || Ok(E::Fr::one()))?;

    let mut exponent_result =
        AllocatedNum::alloc(cs.namespace(|| "allocate exponent result"), || {
            Ok(E::Fr::one())
        })?;
    ...
    let mut exponent_base = AllocatedNum::alloc(cs.namespace(|| "allocate exponent base"), || {
    ...
    for i in 0..exponent_length {
        ...
        exponent_base = exponent_base
            .clone()
            .square(cs.namespace(|| format!("make exponent base {}", i)))?;
...
fn test_parsing() {
    ...
    let exp_result =
        parse_with_exponent_le(cs.namespace(|| "parse"), &circuit_bits, 5, 1, 10).unwrap();
