...
use crate::as_waksman::{AsWaksmanRoute, AsWaksmanTopology, IntegerPermutation};
    ...
    for column_idx in 0..num_columns {
        ...
        for packet_idx in 0..topology.size {
                ...
                == topology.topology[column_idx][packet_idx].1
            {
                ...
                let previous_level_variable = previous_level_variable
                ...
                let new_variable_for_this_level = AllocatedNum::alloc(
                    cs.namespace(|| {
                        format!(
                            "Straight line variable at column {} for packet {}",
                            column_idx, packet_idx
                        )
                    }),
                    || {
                ...
                let switch_value = if a.is_some() { a.cloned() } else { b.cloned() };
                ...
                let cross_value = AllocatedNum::alloc(
                    cs.namespace(|| {
                        format!(
                            "Cross variable at column {} for packet {} with pair {}",
                            column_idx, packet_idx, another_idx
                        )
                    }),
                    || {
                ...
                let straight_value = AllocatedNum::alloc(
                    cs.namespace(|| {
                        format!(
                            "Straight variable at column {} for packet {} with pair {}",
                            column_idx, packet_idx, another_idx
                        )
                    }),
                    || {
                ...
                let (next_level_straight, next_level_cross) = AllocatedNum::conditionally_reverse(
                    cs.namespace(|| {
                        format!(
                            "Perform a switching at column {} for packets {} and {}",
                            column_idx, packet_idx, another_idx
                        )
                    }),
                    &straight_value,
...
#[cfg(test)]
mod test {
    use super::super::num::AllocatedNum;
    ...
    fn test_as_waksman_gadget_positive() {
        ...
        for size in 3..128 {
            ...
            for _ in 0..10 {
                ...
                for (i, (o, p)) in original_vector
                    .into_iter()
                    .zip(permuted_vector.into_iter())
                    .enumerate()
                {
                    let o = AllocatedNum::alloc(
                        cs.namespace(|| format!("original variable {}", i)),
                        || Ok(o),
                    )
                    .unwrap();
                    let p = AllocatedNum::alloc(
                        cs.namespace(|| format!("permuted variable {}", i)),
                        || Ok(p),
                    )
                    .unwrap();
