...
impl<E: Engine> AllocatedNum<E> {
    ...
    pub fn equals<CS>(mut cs: CS, a: &Self, b: &Self) -> Result<boolean::Boolean, SynthesisError>
    where
        E: Engine,
        CS: ConstraintSystem<E>,
    {
        ...
        let delta = Self::alloc(cs.namespace(|| "delta"), || {
        ...
        let delta_inv = Self::alloc(cs.namespace(|| "delta_inv"), || {
        ...
        let t = Self::alloc(cs.namespace(|| "t"), || {
...
impl<E: Engine> Num<E> {
    ...
    pub fn mut_add_number_with_coeff(&mut self, variable: &AllocatedNum<E>, coeff: E::Fr) {
        let newval = match (self.value, variable.get_value()) {
            (Some(mut curval), Some(val)) => {
                let mut tmp = val;
                tmp.mul_assign(&coeff);
    ...
    pub fn mut_add_bool_with_coeff(&mut self, one: Variable, bit: &Boolean, coeff: E::Fr) {
        let newval = match (self.value, bit.get_value()) {
            (Some(mut curval), Some(bval)) => {
                if bval {
                    curval.add_assign(&coeff);
