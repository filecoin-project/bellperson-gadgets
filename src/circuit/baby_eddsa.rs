...
use super::boolean::{field_into_boolean_vec_le, le_bits_into_le_bytes, Boolean};
...
impl<E: JubjubEngine> EddsaSignature<E> {
        ...
        CS: ConstraintSystem<E>,
    {
        ...
        let scalar_bits =
            field_into_boolean_vec_le(cs.namespace(|| "Get S bits"), self.s.get_value())?;
        ...
        let pk_mul_hash = self.pk.mul(cs.namespace(|| "Calculate h*PK"), &h, params)?;
        ...
        CS: ConstraintSystem<E>,
    {
        ...
        let pk_x_serialized = field_into_boolean_vec_le(
            cs.namespace(|| "Serialize PK_X"),
            self.pk.get_x().get_value(),
        )?;
...
mod test {
    ...
    fn test_schnorr_signatures() {
        ...
        let s = AllocatedNum::alloc(cs.namespace(|| "allocate s"), || Ok(sigs_converted)).unwrap();
        ...
        print!(
            "Schnorr signature verification takes constraints: {}\n",
            cs.num_constraints()
        );
    }
    ...
    fn test_valid_musig_signatures() {
        ...
        assert!(cs.is_satisfied());
        print!(
            "MuSig verification without message hashing takes constraints: {}\n",
            cs.num_constraints()
        );
    }
    ...
    fn test_valid_raw_message_signatures() {
        ...
        assert!(cs.is_satisfied());
        print!(
            "EdDSA variant raw message signature takes constraints: {}\n",
            cs.num_constraints()
        );
    }
}
